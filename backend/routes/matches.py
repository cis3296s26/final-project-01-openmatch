...
async def create_post(payload: MatchPostCreate, current_user: dict = Depends(get_current_user)):
    ...
    with engine.begin() as conn:
        ...
        if not verify_user_has_sport_profile(conn, user_id, payload.sport_id):
            raise HTTPException(
                status_code=403,
                detail="You must create sport profile for this team's sport",
            )
        ...
        row = conn.execute(
            text(
                ...
                INSERT INTO match_posts (
                    user_id, team_id, sport_id, title, skill, location, note,
                    expires_at, players_per_side, is_competitive
                )
                VALUES (
                    :user_id, :team_id, :sport_id, :title, :skill, :location, :note,
                    :expires_at, :players_per_side, :is_competitive
                )
                RETURNING id, user_id, team_id, sport_id, title, skill, location, note,
                          status, players_per_side, is_competitive,
        ...
        rows = conn.execute(
            text(
                ...
                SELECT p.id, p.user_id, p.team_id, p.sport_id, p.title, p.skill, p.location, p.note,
                       p.status, p.players_per_side, p.is_competitive,
...
async def update_post(post_id: int, payload: MatchPostUpdate, current_user: dict = Depends(get_current_user)):
    user_id = int(current_user["sub"])

    with engine.begin() as conn:
        existing = conn.execute(
            text("SELECT id, user_id, sport_id, team_id FROM match_posts WHERE id = :post_id"),
            {"post_id": post_id},
        ).mappings().first()
...
async def ready_up(post_id: int, current_user: dict = Depends(get_current_user)):
    ...
    with engine.begin() as conn:
        ...
        if int(sel["total"] or 0) > 0 and int(sel["ready_count"] or 0) == int(sel["total"]):
            conn.execute(
                text(
                    """
                    UPDATE match_posts
